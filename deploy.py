...
def write_env_file(config: dict[str, object]) -> None:
    ...
    content = [
        ...
        f"PROOFORIGIN_SECRETS_BACKEND={config.get('secrets_backend', 'local')}",
    ]
    ...
    if config.get("opentimestamps_endpoint"):
        content.append(f"PROOFORIGIN_OPENTIMESTAMPS_ENDPOINT={config['opentimestamps_endpoint']}")
    vault = config.get("vault", {}) or {}
    if vault.get("addr"):
        content.extend(
            [
                f"VAULT_ADDR={vault['addr']}",
                f"VAULT_TOKEN={vault.get('token', '')}",
                f"PROOFORIGIN_VAULT_MASTER_KEY_PATH={vault.get('master_key_path', '')}",
            ]
        )
